define(["A1Vec2", "A1util"], function(A1Vec2, A1util){
	...
	A1Maze.prototype.init = function(){
		...
		var x = A1util.random(0, (this.width-1))|1;
		var y = A1util.random(0, (this.height-1))|1;
		this.startCarvePos = new A1Vec2(x, y);
	...
	A1Maze.prototype.carveAndRender = function(){
		...
		var curNodeList = this.frontierList.splice(A1util.random(0, this.frontierList.length), 1);
		var curNode = curNodeList[0];
		var nList = this.neighbours(curNode.x, curNode.y);
		var rndNeighbour = nList[A1util.random(0, nList.length)];
	...
	A1Maze.prototype.getPlayerStart = function(){
		...
		while(true){
			...
			x = A1util.random(0, (this.width-1))|1;
			y = A1util.random(0, (this.height-1))|1;
	...
	A1Maze.prototype.placeExit = function(){
			...
			var side = A1util.random(0,2);
			var xSet = A1util.random(0,2);
			var ySet = 1-xSet;
			this.exitPos = new A1Vec2(0, 0);
			var inValid = true;
			...
			while(inValid){
				...
				if(side === 1){
					if(xSet === 1){			//1(1,0)
						...
						this.exitPos.x = A1util.random(1, (this.width-2));
						this.exitPos.y = 0;
					} else {				//1(0,1)
						//this time y is random and x is fixed
						this.exitPos.x = 0;
						this.exitPos.y = A1util.random(1, (this.height-2));
					}
	...
	A1Maze.prototype.addFrontier = function(x, y){
		if((x > 0 && x < (this.width-1)) && (y > 0 && y < (this.height-1)) && (this.field[y][x] === 0)){
			...
			this.field[y][x] = 3;
			this.frontierList.push(new A1Vec2(x, y));
		}
	};
	...
	A1Maze.prototype.neighbours = function(x, y){
		...
		if(this.checkNode(x-2, y)){
			list.push(new A1Vec2(x-2, y));
		}
		if(this.checkNode(x+2, y)){
			list.push(new A1Vec2(x+2, y));
		}
		if(this.checkNode(x, y-2)){
			list.push(new A1Vec2(x, y-2));
		}
		if(this.checkNode(x, y+2)){
			list.push(new A1Vec2(x, y+2));
		}
	...
	A1Maze.prototype.randomTile = function(tileType){
		...
		var out = A1util.random(0, 8).toString();
	...
	A1Maze.prototype.drawImage = function(val, x, y){
		...
		this.core.drawImage(this.resources.getTile(tileType, tileIndex), x, y);
	};
